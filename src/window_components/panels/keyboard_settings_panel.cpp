#include "keyboard_settings_panel.h"

LRESULT KeyboardSettingsPanel::OnCreate()
{
...
LRESULT KeyboardSettingsPanel::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg) {
        case WM_CREATE:
            return OnCreate();
...
KeyboardSettingsPanel *KeyboardSettingsPanel::Create(HWND hWndParent)
{
    Button *self = new Button();
    if (self && self->WinCreateWindow(0,
        //  TEXT("Button"), WS_CHILD | WS_HSCROLL | WS_VSCROLL,
        TEXT("KeyboardSettingsPanel"), WS_CHILD,
...
void KeyboardSettingsPanel::PaintContent(PAINTSTRUCT *pps)
{
